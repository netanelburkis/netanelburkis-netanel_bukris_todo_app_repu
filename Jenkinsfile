...
// The pipeline also includes error handling and notifications via Slack and email.
// The pipeline is designed to be triggered by changes in specific files and branches.
// It uses Docker, Ansible, and GitHub for deployment and version control.
// The pipeline is structured to ensure that only the necessary stages are executed based on the changes detected in the repository.
// The pipeline is designed to be run on a Jenkins server with the necessary plugins and configurations in place.
   agent any
    environment {
